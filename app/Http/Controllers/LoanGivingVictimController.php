...
class LoanGivingVictimController extends Controller
{
    ...
    {
        $query = LoanGivingVictim::with([
            'basicDetailRegistration',
            'debtorDetails',
            'user' => function ($query) {
        ...
        if (!Auth::user()->isSuperSuperAdmin()) {
            if (!Auth::user()->isSuperAdmin()) {
                $query->whereHas('user', function ($subQuery) {
                    $subQuery->where('district_id', Auth::user()->district_id);
                });
            }
        }
    
        $dataTableQuery = $query->get();
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $loanGivingVictims = $this->getForDataTable($request->all());
            return DataTables::of($loanGivingVictims)
                ->addColumn('applicant_name', function ($loanGivingVictims) {
                    return $loanGivingVictims->basicDetailRegistration->applicant_name ?? '';
                })
                ->addColumn('applicant_temp_local', function ($loanGivingVictims) {
                    return $loanGivingVictims->basicDetailRegistration->applicant_temp_local ?? '';
                })
                ->addColumn('applicant_occup', function ($loanGivingVictims) {
                    return $loanGivingVictims->basicDetailRegistration->applicant_occup ?? '';
                })
                ->addColumn('debtor_name', function ($loanGivingVictims) {
                    return $loanGivingVictims->debtorDetails->debtor_name ?? '';
                })
                ->addColumn('debit_date', function ($loanGivingVictims) {
                    return $loanGivingVictims->debtorDetails->debit_date ?? '';
                })
                ->addColumn('debit_amount', function ($loanGivingVictims) {
                    return $loanGivingVictims->debtorDetails->debit_amount ?? '';
                })
                ->addColumn('debit_medium', function ($loanGivingVictims) {
                    return $loanGivingVictims->debtorDetails->debit_medium ?? '';
                })
                ->addColumn('register_by', function ($loanGivingVictims) {
                    return $loanGivingVictims->user->name;
                })
                ->addColumn('created_at', function ($loanGivingVictims) {
                    return $loanGivingVictims->created_at->diffForHumans();
                })
    ...
    {
        //
        $page_title = 'अनुचित लेनदेन (मिटरब्याज) सम्बन्धी ऋण दिने व्यक्तिले भर्नुपर्ने फारम [फारम: २]';
    ...
    public function store(Request $request)
    {
        ...
        $validatedFinanceData = $request->validate([
            ...
            'loan_giving_applicant_finance_cond.land_area' => 'required|array',
            'loan_giving_applicant_finance_cond.land_area.*' => 'required|string',
        ...
        $validatedData = $request->validate([
            'loan_giving_application_other_details.loan_landrestrict_owner' => 'nullable|string|max:255',
            'loan_giving_application_other_details.loan_taking_person_name' => 'nullable|string|max:255',
            'loan_giving_application_other_details.land_passed_name' => 'nullable|string|max:255',
            'loan_giving_application_other_details.registered_person_relation' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_kitta.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_kitta' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_area.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_area' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_state.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_state' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_district.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_district' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_local.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_local' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_ward.*' => 'nullable|string|max:255',
            'loan_giving_application_other_details.landrestrict_ward' => 'nullable|array|max:255',
            'loan_giving_application_other_details.landrestrict_registration_date' => 'nullable|string|max:255',
            ...
            'loan_giving_application_other_details.landrestricted_usedby_now' => 'nullable|string',
    ...
    // }
        ...
        $OtherDetail = null;
            ...
            if ($request->has('loan_giving_application_other_details')) {
                ...
                $OtherDetail = LoanGivingApplicationOtherDetails::create([
                    'loan_landrestrict_owner' => $loanOthersData['loan_landrestrict_owner'] ?? null,
                    'loan_taking_person_name' => $loanOthersData['loan_taking_person_name'] ?? null,
                    'land_passed_name' => $loanOthersData['land_passed_name'] ?? null,
                    'registered_person_relation' => $loanOthersData['registered_person_relation'] ?? null,
                    'landrestrict_kitta' => isset($loanOthersData['landrestrict_kitta']) ? implode(',', $loanOthersData['landrestrict_kitta']) : null,
                    'landrestrict_area' => isset($loanOthersData['landrestrict_area']) ? implode(',', $loanOthersData['landrestrict_area']) : null,
                    'landrestrict_state' => isset($loanOthersData['landrestrict_state']) ? implode(',', $loanOthersData['landrestrict_state']) : null,
                    'landrestrict_district' => isset($loanOthersData['landrestrict_district']) ? implode(',', $loanOthersData['landrestrict_district']) : null,
                    'landrestrict_local' => isset($loanOthersData['landrestrict_local']) ? implode(',', $loanOthersData['landrestrict_local']) : null,
                    'landrestrict_ward' => isset($loanOthersData['landrestrict_ward']) ? implode(',', $loanOthersData['landrestrict_ward']) : null,
                    'landrestrict_registration_date' => $loanOthersData['landrestrict_registration_date'] ?? null,
        ...
        if ($DebtorDetail) {
            ...
            return redirect()->route('admin.loangiving-victim.index')->with('success', 'Data saved successfully.');
    ...
    public function show($id)
    {
        ...
        $previouslyStoredCourtValue = (bool) $loanGivingDebtorApplicationDetail->is_statement_register;
        ...
        return view('admin.LoanGivingVictim.show', compact(
            'loanGivingVictim', 'states', 'previouslyStoredCourtValue', 'previouslyStoredValueForChequeShown',
...
private function getNameFunction($id, $class)
{
    ...
    public function edit(LoanGivingVictim $loanGivingVictim, $id)
    {
        ...
        $purposes = Purpose::select('title', 'id')->get();
        $loanGivingDebtorApplicationDetail = LoanGivingDebtorApplicationDetail::findOrFail($id);
        $previouslyStoredCourtValue = $loanGivingDebtorApplicationDetail->is_statement_register;
